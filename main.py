...
class GameScreen(Screen):
    ...
    def reset_ui(self, *args):
        ...
        container.add_widget(Factory.StyledButton(text="ЛЕГКИЙ (1-10)", on_press=lambda x: self.start_game(10, 5)))
        container.add_widget(Factory.StyledButton(text="СЛОЖНЫЙ (1-100)", on_press=lambda x: self.start_game(100, 7)))
        container.add_widget(Factory.StyledButton(text="В МЕНЮ", on_press=lambda x: setattr(self.manager, 'current', 'menu')))
    ...
    def start_game(self, max_range, attempts):
        ...
        container.add_widget(Factory.StyledButton(text="ПРОВЕРИТЬ", on_press=self.check_input))
    ...
    def check_input(self, instance):
        ...
        if status in ["win", "lose"]:
            ...
            self.ids.container.add_widget(Factory.StyledButton(text="ИГРАТЬ СНОВА", on_press=self.reset_ui))
        else:
...
class ScoresScreen(Screen):
    def update_table(self):
        ...
        container.add_widget(Factory.StyledButton(text="НАЗАД", on_press=lambda x: setattr(self.manager, 'current', 'menu')))
